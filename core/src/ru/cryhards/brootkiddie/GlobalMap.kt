 ...
 */
object GlobalMap {
    val map = ImageActor("map.jpg")

    fun highlightRegion(region: Regions): ImageActor {
        val reg = Spawner.getRegionOverlappingSquare()
    ...
    fun mapCoordinates(region: Actor): Pair<Float, Float> {
        return Pair(
                (region.x + region.width/2 - map.x - map.width/2) / (map.width/2),
                (region.y + region.height/2 - map.y - map.height/2) / (map.height/2)
        )
    }

    fun unmapCoordinates(region: Regions): Pair<Float, Float> {
        return Pair(
                map.x + map.width/2 + region.x * map.width/2,
                map.y + map.height/2 + region.y * map.height/2
        )
    }

    fun mapSize(region: Actor): Pair<Float, Float> {
        return Pair(
