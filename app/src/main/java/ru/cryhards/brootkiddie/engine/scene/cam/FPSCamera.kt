...
class FPSCamera: Viewable {
    ...
    override fun getMatrix(): FloatArray {
        ...
        val camRotationMatrix = MoreMatrix.getLookAroundRotationM(rotation.horizontal.value!!, rotation.vertical.value!!)
        val camTransitionMatrix = MoreMatrix.getTranslationM(position.x.value!!, position.y.value!!, position.z.value!!)
